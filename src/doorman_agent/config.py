    ...
    if config_path and os.path.exists(config_path):
        if not YAML_AVAILABLE:
            ...
            config_data = {
                ...
                'celery_broker_url': yaml_config.get('celery_broker_url', 'redis://localhost:6379/0'),
                'celery_app_name': yaml_config.get('celery_app_name', 'tasks'),
                'check_interval_seconds': yaml_config.get('check_interval_seconds', 30),
                'monitored_queues': yaml_config.get('monitored_queues', ['celery', 'default', 'priority', 'emails', 'payments']),
            }
